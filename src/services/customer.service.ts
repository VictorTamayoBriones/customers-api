...
export const updateCustomerById = async (id: string, params:any) =>{
    ...
    try{
        ...
        if(result[0].affectedRows > 0){
            const result:any = await pool.query("INSERT INTO versions(id, id_customer, full_name, nss, rfc, phone, address) VALUES (?,?,?,?,?,?,?)", [UUID(), id, tempData.full_name, tempData.nss, tempData.rfc, tempData.phone, tempData.address]);
            console.log(result)
        }
